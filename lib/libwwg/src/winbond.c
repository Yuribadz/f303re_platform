...
void
w25_spi_setup(
  __attribute__((unused))uint32_t spi,		// SPI1 or SPI2
  __attribute__((unused))bool bits8,		// True for 8-bits else 16-bits
  __attribute__((unused))bool msbfirst,	// True if MSB first else LSB first
  __attribute__((unused))bool mode0,		// True if mode 0 else mode 3
  __attribute__((unused))uint8_t fpclk_div	// E.g. SPI_CR1_BAUDRATE_FPCLK_DIV_256
) {
#ifndef STM32F3
